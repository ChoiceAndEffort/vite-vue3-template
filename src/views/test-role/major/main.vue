 ...
 * @LastEditors: lg
 * @LastEditTime: 2024-08-09 09:10:18
 * @Description:
 * @FilePath: \vite-vue3-template\src\views\test-role\major\main.vue
-->
<template>
  <el-table
    :data="tableData"
...
const props = defineProps({
  ...
  currentCheckedPowers: {
    type: Array,
    default: () => []
  }
});
watch(
  () => props.currentCheckedPowers,
  (nv) => {
    console.log('----------first', nv);
    tableData.value = cloneDeep(traverseAndModify(props.powerTree));
    // tableData.value = nv;
    lastCheckedData.value = nv;
    if (tableData.value?.length && nv?.length) {
      // debugger
      displaySelection();
    }
  },
